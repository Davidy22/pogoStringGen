...
def login():
	...
	user = requests.get(url = "https://www.googleapis.com/oauth2/v3/userinfo", params = {"access_token": access_token}).json()
	print(user)
	uid = user["sub"]
	...
	if data is None:
		...
		if user["email_verified"]:
			...
			return render_template('new_account.html', uid = uid, username = email[:email.find("@")], state = state, picture = picture)
		else:
		...
		return render_template('index.html', pokemon = pokemonList, username = data["username"], text = f["text"], select = f["select"], image = data["image"], sid = sid)
...
def save():
	...
	try:
		...
		if data is None: raise Exception("Session not found")
	except:
...
def check_login():
	...
	if uid is None:
		...
		file = open(f"pastes/{data['textloc']}.json", "r")
		f = json.load(file)
		return jsonify(result = True, username = data["username"], text = f["text"], select = f["select"], image = data["image"])
...
def view(username):
	...
	return render_template("view.html", pokemon = [pokemon[str(i)] for i in parse(f["select"])], username = username, text = f["text"])
